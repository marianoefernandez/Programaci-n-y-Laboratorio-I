...
/*
int* validarFecha(char* laFecha)
{
    char diaAux[3];
    char mesAux[3];
    char anioAux[5];
    int fechaInt[3]; //0=dia 1=mes 2=anio
    ...
    fechaInt[0]=atoi(diaAux);
    fechaInt[1]=atoi(mesAux);
    fechaInt[2]=atoi(anioAux);

    printf("\n%d/%d/%d", fechaInt[0],fechaInt[1],fechaInt[2]);
...
int isADate(char* fecha)
{
    int i=0;

    while(fecha[i]!='\0')
    {
        if (fecha[0]== '/' || fecha[6] == '/' || fecha[7] == '/' || fecha[8] == '/' || fecha[9] == '/' && i>9)
            return 0;
        i++;
    }
